...
impl FileLoader for TreeFileLoader {
    async fn load(
            ...
            .and_then(|files| files.resolve_trivial(SameChange::Accept))
        {
            ...
            None => {
                // conflict path — pick the first available file version.
                // This is consistent with Git's behavior (conflicted
                // .gitattributes is undefined behavior).
                ...
                tracing::warn!(
                    path = ?path,
                    ".gitattributes is in conflict; using first available version"
                );
                id
            }
        };
...
struct GitAttributesNode {
    disk_first: Mutex<Option<Arc<SearchAndCollection>>>,
    store_first: Mutex<Option<Arc<SearchAndCollection>>>,
    disk_file_loader: Arc<dyn FileLoader>,
    store_file_loader: Arc<dyn FileLoader>,
...
impl GitAttributesNode {
    ...
    fn store(&self, priority: SearchPriority) -> &Mutex<Option<Arc<SearchAndCollection>>> {
        match priority {
    ...
    async fn primary_then_secondary(
        ...
        if let Some(value) = self.store(priority).lock().unwrap().as_ref() {
            return Ok(Arc::clone(value));
        }
        // Slow path: acquire lock and initialize
        let result = self.init_search_and_collection(priority).await?;
        let mut guard = self.store(priority).lock().unwrap();
        if guard.is_none() {
    ...
    async fn init_search_and_collection(
        &self,
        priority: SearchPriority,
    ) -> Result<Arc<SearchAndCollection>, GitAttributesError> {
...
impl GitAttributes {
    ...
    fn inner(
        ...
        let new_node = Arc::new(GitAttributesNode {
            disk_first: Mutex::new(None),
            store_first: Mutex::new(None),
...
impl GitAttributes {
    /// Returns the filter name if the path has a `filter` attribute set in
    /// `.gitattributes`.
    pub(crate) async fn filter_name(
        &self,
        path: &RepoPath,
        priority: SearchPriority,
    ) -> Result<Option<String>, GitAttributesError> {
        let result = self.search(path, ["filter"], priority).await?;
        let Some(State::Value(value)) = result.get("filter") else {
            return Ok(None);
        };
        Ok(Some(value.as_ref().as_bstr().to_string()))
    }

    /// Returns whether the given `path` has a `filter` attribute in
    /// .gitattributes whose value is contained in `ignore_filters`.
    ///
    /// This is the foundation for the "ignore filters" approach (Route A in
    /// `PLAN.lfs-support.md`), where files matching certain filter values (e.g.
    /// `lfs`) are skipped entirely during snapshot and checkout. Currently
    /// unused in production — the clean/smudge approach (Route B) is used
    /// instead — but kept as a tested building block for a future hybrid mode
    /// (Route C) where users could opt into ignoring LFS files.
    #[allow(dead_code)]
...
mod tests {
    ...
    #[test]
    fn test_filter_name_unset() {
        // `-filter=lfs` unsets the filter attribute
        let data = TestStore::from([(
            repo_path(".gitattributes").to_owned(),
            Ok("*.bin -filter=lfs\n".to_string()),
        )]);
        let git_attributes = GitAttributes::new(HashMap::new(), data);
        let result = git_attributes
            .filter_name(repo_path("file.bin"), SearchPriority::Disk)
            .block_on()
            .unwrap();
        assert_eq!(result, None, "-filter should return None");
    }

    #[test]
    fn test_filter_name_unspecified() {
        // `!filter=lfs` marks the filter as unspecified
        let data = TestStore::from([(
            repo_path(".gitattributes").to_owned(),
            Ok("*.bin !filter=lfs\n".to_string()),
        )]);
    ...
    #[test]
    fn test_filter_name_override_to_none() {
        // A more specific .gitattributes can unset the filter
        let data = TestStore::from([
            (
                repo_path(".gitattributes").to_owned(),
                Ok("*.bin filter=lfs\n".to_string()),
        ...
        let result = git_attributes
            ...
            .block_on()
            .unwrap();
        assert_eq!(result.as_deref(), Some("lfs"));
        // Subdir: filter is unset by override
        let result = git_attributes
            .filter_name(repo_path("subdir/file.bin"), SearchPriority::Disk)
            .block_on()
            .unwrap();
        assert_eq!(result, None, "subdir override should unset filter");
    }

    #[test]
    fn test_filter_name_via_macro() {
        // A macro attribute can set the filter
        let data = TestStore::from([(
            repo_path(".gitattributes").to_owned(),
            Ok("[attr]lfs-files filter=lfs\n*.bin lfs-files\n".to_string()),
        )]);
        let git_attributes = GitAttributes::new(HashMap::new(), data);
        let result = git_attributes
            .filter_name(repo_path("file.bin"), SearchPriority::Disk)
        ...
        assert_eq!(
            result.as_deref(),
            Some("lfs"),
            "filter should be set via macro expansion"
        );
    }
}
